...
def topk(split_path, label_reader, k, conf_thres, window, window_samples, batch_size, max_score, ref_dist, data_size, cached_gt_path, gtod_fps):
    cdf_time = 0
    selection_time = 0
    update_bound_time = 0
    gtod_time = 0
    ...
    prev = time.time()
    pi, mu, sigma = load_distribution(split_path, label_reader, data_size, window, ref_dist)
    cdf = gen_cdf(pi, mu, sigma, max_score, 1e5)
    cdf_time = time.time() - prev

    num_frames_cleaned = 0

    if window < 2:
        remained_ref = load_np_by_prefix(split_path, "remained_")
        train_idx = np.load(os.path.join(split_path, "train_idxs.npy"))
        val_idx = np.load(os.path.join(split_path, "valid_idxs.npy"))

        initial_certain = np.concatenate([train_idx, val_idx], 0)
        initial_certain_scores = infer_frame_gt(initial_certain, label_reader)
    ...
    update_bound_time += time.time() - prev
    ...
    while select_heap.size > 0:
        ...
        selection_time += time.time() - prev

        if window < 2:
            scores = infer_frame_gt(f2idx(idx_list, clean_f), label_reader)
            num_frames_cleaned += len(scores)
            ...
            scores = infer_window_gt(clean_f, window, window_samples, label_reader, data_size)
            num_frames_cleaned += len(scores) * window_samples
    ...
    infer_time = num_frames_cleaned / 112

    precision, rank_dist, score_error = evaluate(topk, k, label_reader, window, data_size, cached_gt_path)
    print("[EXP]cdf_time:", cdf_time)
    print("[EXP]selection_time:", selection_time)
    print("[EXP]update_bound_time:", update_bound_time)
    print("[EXP]gtod_time:", infer_time)
    print("[EXP]topk_time:", cdf_time + selection_time + update_bound_time + infer_time)
    print("[EXP]num_cleaned:", num_frames_cleaned)
